...
class ImageHistoryBuffer(object):
    def __init__(self, shape, max_size, batch_size):
        ...
        """
        self.image_history_buffer = None  # np.zeros(shape, dtype=np.float)
    ...
    def add_to_image_history_buffer(self, images, nb_to_add=None):
        ...
        if isinstance(self.image_history_buffer, type(None)):
            print('Initialize image history buffer')
            self.image_history_buffer = images[:nb_to_add]
